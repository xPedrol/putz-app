...
import {CookieOptions, ICookieWriterService} from '../browser/cookie.model';
import {isEmpty, isNil, isString} from '../browser/utils';
...
export class CookieBackendWriterService implements ICookieWriterService {
  ...
  private getNormalizedResponseCookies(): string[] {
    ...
    if (this.response && (this.response.getHeader('Set-Cookie') as string | string[])) {
      responseCookies = (this.response.getHeader('Set-Cookie') as string | string[]);
    }
